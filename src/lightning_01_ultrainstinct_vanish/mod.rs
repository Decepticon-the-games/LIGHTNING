...
pub static mut YOU_X : [f32; 8] = [0.0; 8];
pub static mut YOU_Y : [f32; 8] = [0.0; 8];
pub static mut YOU_BOMA : [u64; 8] = [0; 8];
...
move_type_again: bool) -> u64 {
    ...
    //VANISH (ATTACKER)
        if utility::get_category(&mut *attacker_boma) == *BATTLE_OBJECT_CATEGORY_FIGHTER { // if you're the attacker
            if ACTIVATE_VANISH[a_entry_id] 
            && AttackModule::is_attack_occur(&mut *attacker_boma) 
            {
                if utility::get_category(&mut *defender_boma) == *BATTLE_OBJECT_CATEGORY_FIGHTER 
                || utility::get_category(&mut *defender_boma) == *BATTLE_OBJECT_CATEGORY_ENEMY { // if the one being attacked (defender) is a fighter/enemy
                    OPPONENT_BOMA[a_entry_id] = (&mut *defender_boma as *mut BattleObjectModuleAccessor) as u64; 
                    OPPONENT_X[a_entry_id] = PostureModule::pos_x(defender_boma); //get defender's x position
                    OPPONENT_Y[a_entry_id] = PostureModule::pos_y(defender_boma); //get defender's y position
                    if utility::get_category(&mut *defender_boma) == *BATTLE_OBJECT_CATEGORY_FIGHTER {
                    ...
                    let oboma = sv_battle_object::module_accessor((WorkModule::get_int(attacker_boma, *WEAPON_INSTANCE_WORK_ID_INT_LINK_OWNER)) as u32);
                    if utility::get_category(&mut *oboma) != *BATTLE_OBJECT_CATEGORY_FIGHTER {
                        OPPONENT_X[a_entry_id] = PostureModule::pos_x(oboma);
                        OPPONENT_Y[a_entry_id] = PostureModule::pos_y(oboma);
                        OPPONENT_BOMA[a_entry_id] = (&mut *attacker_boma as *mut BattleObjectModuleAccessor) as u64;
                    }
                    else {
                        OPPONENT_X[a_entry_id] = PostureModule::pos_x(defender_boma);
                        OPPONENT_Y[a_entry_id] = PostureModule::pos_y(defender_boma);
                        OPPONENT_BOMA[a_entry_id] = (&mut *defender_boma as *mut BattleObjectModuleAccessor) as u64;
                        if utility::get_category(&mut *defender_boma) == *BATTLE_OBJECT_CATEGORY_FIGHTER {
                            JostleModule::set_status(&mut *defender_boma, false);
                        }
                    ...
                    OPPONENT_X[a_entry_id] = PostureModule::pos_x(attacker_boma);
                    OPPONENT_Y[a_entry_id] = PostureModule::pos_y(attacker_boma);
                    OPPONENT_BOMA[a_entry_id] = (&mut *attacker_boma as *mut BattleObjectModuleAccessor) as u64;
                }
            ...
            if entry_id < 8 {
                ...
                if SECRET_SENSATION[entry_id] {
                    ...
                    if CAMERA[entry_id] == false { // Exists so all of this code will only happen once.
                        ...
                        YOU_X[entry_id] = PostureModule::pos_x(boma); // Gets Ryu's position
                        YOU_Y[entry_id] = PostureModule::pos_y(boma);
                        if YOU_X[entry_id] == OPPONENT_X[entry_id] { // The reason I sometimes set Ryu's position as the Opponent's position is for this, if Ryu can't find the owner of what hit him, he will instead just dodge backwards.
                        ...
                        else if YOU_X[entry_id] < OPPONENT_X[entry_id] { // Checks where Ryu and his Opponent are relative to each other, and sets a value so Ryu always moves *behind* the opponent
                        ...
                        if (YOU_Y[entry_id] - OPPONENT_Y[entry_id]).abs() <= 12.0 // Checks to see if Ryu and his opponent are "close enough" in Y value to do the grounded slide instead
                    ...
                    if SEC_SEN_TIMER[entry_id] >= 0.0 { // This whole if statement is for linearly interpolating Ryu's position, instead of just teleporting him behind the opponent.
                        if (YOU_Y[entry_id] - OPPONENT_Y[entry_id]).abs() > 12.0 { // If Ryu's Y and the Opponent's Y are far enough apart, do the following:
                        ...
                        PostureModule::set_pos_2d(boma, &Vector2f{ // Linear Interpolation formula: Destination * t + Starting * (1.0 - t), where 0 <= t <= 1. You can't add vectors apparently, so I did this for both X and Y.
                            ...
                            y: (((OPPONENT_Y[entry_id] + VERT_EXTRA[entry_id]) * SEC_SEN_TIMER[entry_id]) + YOU_Y[entry_id] * (1.0 - SEC_SEN_TIMER[entry_id])) // There's a +12.0 so that, for moving into the air, Ryu moves slightly above the opponent. Does nothing on the ground. I may change this later.
                        });
                    }
        ...
        unsafe {
            ...
            if entry_id < 8 {
                ...
                if VANISH[entry_id] {
                    ...
                    JostleModule::set_status(boma, false); // Turns off body blocking for Ryu every frame Secret Sensation is true
                    macros::WHOLE_HIT(fighter, *HIT_STATUS_XLU); // Makes Ryu invincible.
                    MotionModule::set_rate(boma, 0.001);
                    ...
                    if CAMERA[entry_id] == false { // Exists so all of this code will only happen once.
                        ...
                        && StatusModule::situation_kind(OPPONENT_BOMA[entry_id] as *mut smash::app::BattleObjectModuleAccessor) == *SITUATION_KIND_GROUND {
                            ...
                            StatusModule::set_situation_kind(boma, smash::app::SituationKind(*SITUATION_KIND_AIR), false); // Sets Ryu to airborne
                            WorkModule::on_flag(boma, *FIGHTER_STATUS_WORK_ID_FLAG_RESERVE_GRAVITY_STABLE_UNABLE); // Turns of Gravity for Ryu
                            VERT_EXTRA[entry_id] = 12.0; // Makes Ryu dodge above the opponent, if they're far enough apart in Y values
                            YOU_Y[entry_id] += 2.0; // Sets Ryu's position to be slightly higher than on the ground, so he can do his aerial Focus Attack Dash animation.
                            PostureModule::add_pos_2d(boma, &Vector2f{
                    ...
                    if VANISH_TIMER[entry_id] >= 0.0 { // This whole if statement is for linearly interpolating Ryu's position, instead of just teleporting him behind the opponent.
                        if (YOU_Y[entry_id] - OPPONENT_Y[entry_id]).abs() > 12.0 { // If Ryu's Y and the Opponent's Y are far enough apart, do the following:
                            StatusModule::set_situation_kind(boma, smash::app::SituationKind(*SITUATION_KIND_AIR), false); // Set Ryu to be airborne
                        }
                        PostureModule::set_pos_2d(boma, &Vector2f{ // Linear Interpolation formula: Destination * t + Starting * (1.0 - t), where 0 <= t <= 1. You can't add vectors apparently, so I did this for both X and Y.
                            x: (((OPPONENT_X[entry_id] + OPPONENT_DIRECTION[entry_id]) * VANISH_TIMER[entry_id]) + YOU_X[entry_id] * (1.0 - VANISH_TIMER[entry_id])),
                            y: (((OPPONENT_Y[entry_id] + VERT_EXTRA[entry_id]) * VANISH_TIMER[entry_id]) + YOU_Y[entry_id] * (1.0 - VANISH_TIMER[entry_id])) // There's a +12.0 so that, for moving into the air, Ryu moves slightly above the opponent. Does nothing on the ground. I may change this later.
                        });
                    }
...
pub fn install() {
    ...
    smashline::install_agent_frame_callbacks!(ultrainstinct, vanish);
}