...
fn once_per_fighter_frame_1(fighter : &mut L2CFighterCommon) {
    ...
    let frame = MotionModule::frame(module_accessor);
        ...
        //FIXES
        //-------------------------------------------------------------------------------
        if (status_kind == *FIGHTER_MIIFIGHTER_STATUS_KIND_SPECIAL_N2_ATTACK) && frame >= 17.0 
        || (status_kind == *FIGHTER_MIIFIGHTER_STATUS_KIND_SPECIAL_S1_END) && frame >= 25.0
        {
        ...
        && ! (status_kind == *FIGHTER_MIIFIGHTER_STATUS_KIND_SPECIAL_N2_ATTACK)
...
fn once_per_fighter_frame_3(fighter : &mut L2CFighterCommon) {
    ...
    let frame = MotionModule::frame(module_accessor);
        ...
        //FIXES
        //-------------------------------------------------------------------------------
        if (status_kind == *FIGHTER_MIISWORDSMAN_STATUS_KIND_SPECIAL_N3) && frame >32.0  
        || (status_kind == *FIGHTER_MIISWORDSMAN_STATUS_KIND_SPECIAL_N3_MAX) && frame >32.0  
        {
            if AttackModule:: is_attack_occur(fighter.module_accessor) && ! SlowModule::is_slow(module_accessor){
                if  jump_guard_dash_upspecial_pressed {
                    CancelModule::enable_cancel(module_accessor);
        ...
        && ! (status_kind == *FIGHTER_MIISWORDSMAN_STATUS_KIND_SPECIAL_N3)
        && ! (status_kind == *FIGHTER_STATUS_KIND_ATTACK)
        && ! (status_kind == *FIGHTER_STATUS_KIND_ATTACK_100)
        && ! (status_kind == *FIGHTER_STATUS_KIND_THROW) {
            if AttackModule:: is_attack_occur(fighter.module_accessor) && ! SlowModule::is_slow(module_accessor){
