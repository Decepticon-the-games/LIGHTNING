...
#[acmd_script( agent = "kamui", script = "game_specialairnend2", category = ACMD_GAME, low_priority )]
unsafe fn game_specialairnend2(fighter: &mut L2CAgentBase) {
    let entry_id = WorkModule::get_int(fighter.module_accessor, *FIGHTER_INSTANCE_WORK_ID_INT_ENTRY_ID) as usize;
    CANCEL_IN_NEUTRAL[entry_id] = false;
    frame(fighter.lua_state_agent, 1.0);
    if macros::is_excute(fighter) {
...
unsafe fn game_dhspecialairnend2(fighter: &mut L2CAgentBase) {
    let entry_id = WorkModule::get_int(fighter.module_accessor, *FIGHTER_INSTANCE_WORK_ID_INT_ENTRY_ID) as usize;
...
pub fn install() {
    ...
    game_dhspecialairnend1, game_dhspecialairnend2, game_dhspecialairnendmax);
}